...
function AppContent() {
  ...
  return (
    <div className="size-full flex bg-gray-50">
      {/* Sidebar */}
      <aside
        className={`${sidebarCollapsed ? 'w-16' : 'w-64'} bg-[#0D1B4B] text-white flex flex-col transition-all duration-300 relative`}
      >
        ...
        <button
          ...
          className="absolute -right-3 top-20 w-6 h-6 bg-[#C9A84C] rounded-full flex items-center justify-center hover:bg-[#b89640] transition-colors z-10"
        >
          {sidebarCollapsed ? <ChevronRight className="w-4 h-4 text-[#0D1B4B]" /> : <ChevronLeft className="w-4 h-4 text-[#0D1B4B]" />}
        </button>
        ...
        <nav className="flex-1 p-4 overflow-y-auto">
          {menuItems.map((item, index) => {
            if ('divider' in item && item.divider) {
              ...
              if (sidebarCollapsed) return <div key={index} className="my-2 border-t border-white/10" />;
              return (
            ...
            return (
              <button
                ...
                className={`w-full text-left px-4 py-3 rounded-lg mb-1 transition-colors ${
                  currentScreen === item.key ? 'bg-[#C9A84C] text-[#0D1B4B] font-medium' : 'hover:bg-white/10'
                }`}
        ...
        <div className="p-4 border-t border-white/10">
          {!sidebarCollapsed ? (
            ...
            <button
              onClick={logout}
              className="w-full flex items-center justify-center px-2 py-2 bg-red-600 hover:bg-red-700 rounded-lg transition-colors"
              title="Cerrar Sesión"
            >
              <LogOut className="w-4 h-4" />
            </button>
